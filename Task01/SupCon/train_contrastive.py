...
def prepare_data(
    ...
    # Class 0, augmented: sample 50% with random_state, only if we are not 
    # using raw only
    if not use_raw_only:
...
def get_transforms_specific(
    ...
    if stage == "train" and class_name is not None:
        ...
        if class_name == "Noise":
            ...
            # Spatial + very mild intensity + rare elastic (zipper can have 
            # slight geometric component)
            safe_intensity = [
        ...
        transforms = base_transforms + aug_transforms + [
            ...
            SpatialPadd(keys=["img"], method="symmetric",
...
def get_transforms(
    ...
    if stage == "train":
        transforms = base_transforms + [
            ...
            RandShiftIntensityd(keys=["img"], prob=0.4, offsets=0.1),
            RandScaleIntensityd(keys=["img"], prob=0.4, factors=0.1),
            RandRotated(
                keys=["img"], prob=0.4,
            ...
            RandAffined(
                keys=["img"],
                prob=0.5,
                scale_range=(0.15, 0.15, 0.15),  # Less scaling in slice dir
                translate_range=(5, 5, 5),       # Less translation in slice dir
                mode="bilinear",
                padding_mode="border"
            ),
            CenterSpatialCropd(
...
def train_model(
    ...
    """
    Train the ordinal contrastive model using progressive learning strategy.
    """
    ...
    # train_transforms = get_transforms_specific(config['spatial_size'], 
    #                                           "train", class_name)
    # val_transforms = get_transforms_specific(config['spatial_size'], 
    #                                         "val", class_name)
    ...
    if load_model_path is not None:
        ...
        model = OrdinalContrastiveLightningModule(
            ...
            projection_dim=config['model_params']['projection_dim'],
            ordinal_embedding_dim=config['model_params'][
                'ordinal_embedding_dim'],
            learning_rate=config['learning_rate'],
            weight_decay=config['model_params']['weight_decay'],
            max_epochs=config['num_epochs'],
            contrastive_strategy=config['model_params'][
                'contrastive_strategy'],
            temperature=config['model_params']['temperature'],
            minority_threshold=config['model_params']['minority_threshold'],
            ordinal_weight_scale=config['model_params'][
                'ordinal_weight_scale'],
            contrastive_weight=config['model_params']['contrastive_weight'],
            classification_weight=config['model_params'][
                'classification_weight'],
            ordinal_weight=config['model_params']['ordinal_weight'],
            ordinal_contrastive_weight=config['model_params'][
                'ordinal_contrastive_weight'],
            warmup_epochs=config['model_params']['warmup_epochs'],
            joint_epochs=config['model_params']['joint_epochs'],
            finetune_epochs=config['model_params']['finetune_epochs'],
            dropout_prob=config['model_params']['dropout_prob'],
            use_ensemble_prediction=config['model_params']['use_ensemble_prediction']
        )
    ...
    # Use the evaluate_model method from OrdinalContrastiveLightningModule if available
    ...
    collect_predictions_contrastive(model, eval_dataloaders, config['device'])
    ...
    try:
        visualize_embeddings(
            model, eval_dataloaders, config['device'], 
            config['results_dir'], class_name, wandb_logger
        )
        print(f"Embedding visualizations saved to {config['results_dir']}")
    except Exception as e:
        print(f"Warning: Failed to generate embedding visualizations: {e}")
    # --- END EMBEDDING VISUALIZATION ---

    return model, trainer, evaluation_results


def collect_predictions_contrastive(model, dataloaders, device):
    """Collect predictions for confusion matrix plotting."""
    model.eval()
...
def main():
    ...
    config = {
        ...
        "augmented_dir": "data/LISA2025/BIDS_norm/derivatives/augmented_FINAL",
        "augmented_qc_csv_path": (
            "data/LISA2025/BIDS_norm/derivatives/augmented_FINAL/augmented_labels.csv"
        ),
        "spatial_size": (150, 150, 150),
        "batch_size": 8,
        "num_epochs": 170,  # Total epochs for progressive training
        "learning_rate": 1e-4,
        "random_state": 42,
        "results_dir": "results_contrastive_progressive_final",
        "num_workers": 2,
        "wandb_project": "lisa2025-ordinal-contrastive-progressive",
        "model_params": {
            "num_classes": 3,
            "spatial_dims": 3,
            "in_channels": 1,
            "projection_dim": 64,
            "ordinal_embedding_dim": 32,
            "weight_decay": 1e-5,
            "contrastive_strategy": "combined",
            "temperature": 0.07,
            "minority_threshold": 0.25,
            "ordinal_weight_scale": 2.0,
            "contrastive_weight": 1.0,
            "classification_weight": 0.5,
            "ordinal_weight": 0.3,
            "ordinal_contrastive_weight": 0.8,
            "warmup_epochs": 50,
            "joint_epochs": 100,
            "finetune_epochs": 30,
            "dropout_prob": 0.0,
