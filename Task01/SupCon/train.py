...
class CustomReorientByShapeD(MapTransform):
    """
    Reorient image so smallest dimension is always at target_axis.
    """
    ...
    def __init__(self, 
        ...
        Args:
            ...
            target_axis: axis where smallest dimension should be moved
                (default: 2 for depth)
    ...
    def __call__(self, data: Mapping[Hashable, torch.Tensor]) -> Dict[
        ...
        for key in self.key_iterator(d):
            img = d[key]
            # Get spatial dimensions (assuming first dim is channel)
            spatial_shape = img.shape[1:]  # Skip channel dimension
            # Find axis with smallest dimension
            min_axis = np.argmin(spatial_shape) + 1  # +1 to account for channel dim
            # If smallest dimension is not at target position, transpose
            if min_axis != self.target_axis:
                # Create transpose axes
                axes = list(range(len(img.shape)))
                axes[min_axis], axes[self.target_axis] = (
                    axes[self.target_axis], axes[min_axis])
                # Transpose
                d[key] = img.permute(axes)
        return d
...
def prepare_data(
    ...
    print(
        "Combined samples after filtering: ",
        len(combined_df)
    )
    ...
    # Filter for specific task (single-task learning, allow at most one other task labeled as 1 or 2)
    other_tasks = [col for col in task_columns if col != class_name]
    other_labeled_count = (
        combined_df[other_tasks].isin([1, 2])
    ).sum(axis=1)
    ...
    # Class 0, augmented: sample 50% with random_state
...
def get_transforms(
    ...
    base_transforms = [
        ...
        # CustomReorientByShapeD(keys=["img"], target_axis=3),
...
class ClassBalancedBatchSampler(torch.utils.data.Sampler):
    def __init__(self, labels, batch_size):
        ...
        self.length = len(labels) // batch_size * batch_size
    ...
    def __iter__(self):
        ...
        batch = []
        while True:
            # Randomly pick which classes get the extra samples for this batch
            extra_classes = []
            if self.remainder > 0:
                extra_classes = random.sample(
                    list(self.classes), self.remainder
                )
            for c in self.classes:
                n_samples = self.samples_per_class
                if c in extra_classes:
                    n_samples += 1
                if len(class_indices[c]) < n_samples:
                    class_indices[c] = self.class_indices[c].copy()
                    random.shuffle(class_indices[c])
                batch.extend([
                    class_indices[c].pop()
...
def train_model(
    ...
    """
    Train the ordinal contrastive model using PyTorch Lightning.
    """
    ...
    config['class_weights'] = (
        ...
        if isinstance(class_weights, torch.Tensor) else class_weights
    )
    ...
    # train_labels = [f["label"] for f in train_files]
    ...
    if load_model_path is not None:
        ...
        model = OrdinalContrastiveLightningModule.load_from_checkpoint(
            load_model_path
        )
        trainer = None
    else:
        ...
        model = OrdinalContrastiveLightningModule(
            ...
            use_prototypes=config['model_params']['use_prototypes'],
            prototype_similarity_threshold=(
                config['model_params']['prototype_similarity_threshold']
            ),
            minority_threshold=config['model_params']['minority_threshold'],
            weight_decay=config['model_params']['weight_decay'],
            max_epochs=config['num_epochs'],
            contrastive_weight=config['model_params']['contrastive_weight'],
            qwk_weight=config['model_params']['qwk_weight'],
        ...
        trainer = pl.Trainer(
            ...
            accumulate_grad_batches=(
                config['trainer_params']['accumulate_grad_batches']
            ),
    ...
    # Create separate evaluation dataloaders with reduced memory requirements
    eval_train_loader = DataLoader(
        train_dataset,
        batch_size=8,  # Reduced batch size for evaluation
        shuffle=False,
        num_workers=2,  # No workers to avoid memory issues
        pin_memory=False,  # Disable pin memory for evaluation
        persistent_workers=False
    )
    ...
    model.collect_predictions(
        eval_dataloaders, config['device']
    )
    
    # Visualize embeddings and log to wandb
    if (
        hasattr(model, 'hparams') and
        getattr(model.hparams, 'contrastive_weight', 0.0) > 0.0
    ):
        ...
        print(
            "[INFO] Skipping embedding visualization (contrastive_weight == 0.0)"
        )

    # Plot and log confusion matrices for train, val, and test splits
...
def main():
    ...
    config = {
        ...
        "augmented_dir": "data/LISA2025/BIDS/derivatives/augmented_paper",
        "augmented_qc_csv_path": (
            "data/LISA2025/BIDS/derivatives/augmented_paper/augmented_labels.csv"
        ),
        ...
        "learning_rate": 3e-4,
        "random_state": 42,
        "results_dir": "results_ordinal_clm_1807",
        "num_workers": 2,
        "wandb_project": "lisa2025-ordinal-clm-1807",
        "model_params": {
            "num_classes": 3,
            "spatial_dims": 3,
            "in_channels": 1,
    ...
    if args.task:
        if args.task not in all_tasks:
            raise ValueError(
                f"Invalid task '{args.task}'. Must be one of: {all_tasks}"
            )
        tasks = [args.task]
